...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController
    {
        ...
        private void GatherInput()
        {
			...
			Vector2 moveInput = _controls.Platformer.Move.ReadValue<Vector2>();
			// Transform the move input to make only forward movements possible.
			moveInput.x = (moveInput.x / 2f) + 0.5f;

			Input = new FrameInput
            {
                JumpDown = _controls.Platformer.Jump.WasPressedThisFrame(),
                JumpHeld = _controls.Platformer.Jump.IsPressed(),
                DashDown = false,
